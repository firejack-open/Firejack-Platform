...
/**
 * Class encapsulates renaming the directory functionality
 */
@Component
@TrackDetails
public class RenameFileDirectoryBroker extends ServiceBroker<ServiceRequest<NamedValues>, ServiceResponse> {

	/**
	 * Modifies the name of directory with the specified path
	 *
	...
	protected ServiceResponse perform(ServiceRequest<NamedValues> request) throws Exception {
		...
		fileStore.renameDirectory(name, path);

		return new ServiceResponse("Directory renamed successfully.", true);
	}
}