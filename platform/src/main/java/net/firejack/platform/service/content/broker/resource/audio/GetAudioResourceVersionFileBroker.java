...
@Component("getAudioResourceVersionFileBroker")
@TrackDetails
public class GetAudioResourceVersionFileBroker
	...
	public ServiceResponse<FileInfo> perform(ServiceRequest<NamedValues> request) throws Exception {
		...
		String audioFilename = (String) request.getData().get("audioFilename");
		ServiceResponse<FileInfo> response = new ServiceResponse<FileInfo>();
		InputStream stream = OPFEngine.FileStoreService.download(OpenFlame.FILESTORE_CONTENT, audioFilename, helper.getAudio(), String.valueOf(resourceId));

		FileInfo fileInfo = new FileInfo();
		fileInfo.setStream(stream);
		setOriginalFilename(audioFilename, fileInfo);
		response.addItem(fileInfo);
		response.setSuccess(true);
		return response;
	}

	private void setOriginalFilename(String audioFilename, FileInfo fileInfo) {
        String sId = audioFilename.substring(0, audioFilename.indexOf("_"));
        if (StringUtils.isNumeric(sId)) {
            Long resourceVersionId = Long.parseLong(sId);
            AudioResourceVersionModel resourceVersion = resourceVersionStore.findById(resourceVersionId);
