...
@Component("saveTextResourceBroker")
@TrackDetails
public class SaveTextResourceBroker
        extends AbstractSaveResourceBroker<TextResourceModel, TextResource> {

    @Autowired
    @Qualifier("textResourceStore")
    private IResourceStore<TextResourceModel> store;

    @Override
    protected IResourceStore<TextResourceModel> getResourceStore() {
        return store;
    }

    @Override
    protected String getSuccessMessage(boolean isNew) {
        return "Text resource has " + (isNew ? "created" : "updated") +  " successfully";
    }
