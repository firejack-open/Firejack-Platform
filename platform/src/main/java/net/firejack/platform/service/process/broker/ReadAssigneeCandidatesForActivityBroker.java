...
@TrackDetails
@Component("readAssigneeCandidatesForActivityBroker")
public class ReadAssigneeCandidatesForActivityBroker extends ListBroker<UserModel, User, SimpleIdentifier<Long>> {
    ...
    protected List<UserModel> getModelList(ServiceRequest<SimpleIdentifier<Long>> request) throws BusinessFunctionException {
        Long activityId = request.getData().getIdentifier();
        List<UserModel> activityAssigneeCandidates = store.findActivityAssigneeList(activityId);
        List<UserModel> result;
        if (activityAssigneeCandidates == null) {
            result = Collections.emptyList();
        } else {
            Set<UserModel> distinctCollection = new HashSet<UserModel>(activityAssigneeCandidates);
