...
@TrackDetails
@Component("associatePackageBroker")
public class AssociatePackageBroker extends ServiceBroker<ServiceRequest<NamedValues>, ServiceResponse> {
	@Autowired
	...
	protected ServiceResponse perform(ServiceRequest<NamedValues> message) throws Exception {
        ...
        RegistryNodeModel mainSystem = system.getMain();
        if (mainSystem != null) {
            system = systemStore.findById(mainSystem.getId());
        }

		packageStore.associate(packageModel, system);
		return new ServiceResponse("Package has been associated successfully.", true);
	}
}
