...
@TrackDetails
@Component("updateUserProfileFieldGroupBroker")
public class UpdateUserProfileFieldGroupBroker extends OPFSaveBroker<UserProfileFieldGroupModel, UserProfileFieldGroup, UserProfileFieldGroup> {
	...
	protected String getSuccessMessage(boolean isNew) {
		return "User profile field group has updated successfully";
	}
	...
	protected List<ValidationMessage> specificArgumentsValidation(ServiceRequest<UserProfileFieldGroup> request) throws RuleValidationException {
		UserProfileFieldGroup userProfileFieldGroup = request.getData();
		List<ValidationMessage> validationMessages = super.specificArgumentsValidation(request);
		UserProfileFieldGroupModel sameUserProfileFieldGroup = store.findUserProfileFieldGroupByRegistryNodeIdAndName(userProfileFieldGroup.getParentId(), userProfileFieldGroup.getName());
		if (sameUserProfileFieldGroup != null && !sameUserProfileFieldGroup.getId().equals(userProfileFieldGroup.getId())) {
