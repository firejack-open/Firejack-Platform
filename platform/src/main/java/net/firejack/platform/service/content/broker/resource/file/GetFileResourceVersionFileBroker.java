...
@Component("getFileResourceVersionFileBroker")
@TrackDetails
public class GetFileResourceVersionFileBroker
	...
	public ServiceResponse<FileInfo> perform(ServiceRequest<NamedValues> request) throws Exception {
		...
		String fileFilename = (String) request.getData().get("fileFilename");
		ServiceResponse<FileInfo> response = new ServiceResponse<FileInfo>();
		InputStream stream = OPFEngine.FileStoreService.download(OpenFlame.FILESTORE_CONTENT, fileFilename, helper.getFile(), String.valueOf(resourceId));

		FileInfo fileInfo = new FileInfo();
		fileInfo.setStream(stream);
		setOriginalFilename(fileFilename, fileInfo);
		response.addItem(fileInfo);
		response.setSuccess(true);
		return response;
	}

	private void setOriginalFilename(String fileFilename, FileInfo fileInfo) {
        String sId = fileFilename.substring(0, fileFilename.indexOf("_"));
        if (StringUtils.isNumeric(sId)) {
            Long resourceVersionId = Long.parseLong(sId);
            FileResourceVersionModel resourceVersion = resourceVersionStore.findById(resourceVersionId);
