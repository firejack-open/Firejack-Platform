...
@Component("createImageResourceVersionByPathBroker")
@TrackDetails
public class CreateImageResourceVersionByPathBroker
        extends CreateImageResourceVersionByLookupBroker {
    ...
    protected ServiceResponse<ImageResourceVersion> perform(ServiceRequest<NamedValues> request) throws BusinessFunctionException {
        String resourcePath = (String) request.getData().get("resourcePath");
        ImageResourceVersion imageResourceVersion = (ImageResourceVersion) request.getData().get("imageResourceVersion");

        try {
            RegistryNodeModel parentModel = findOrCreateFolder(resourcePath);
            if (parentModel != null) {
                ImageResourceModel imageResourceModel = new ImageResourceModel();
                imageResourceModel.setName(SecurityHelper.generateRandomSequence(8));
                ...
                String name = DiffUtils.humanNameFromLookup(resourcePath);
                imageResourceModel.setName(StringUtils.normalize(name) + "-" + imageResourceModel.getId());
                imageResourceStore.save(imageResourceModel);
