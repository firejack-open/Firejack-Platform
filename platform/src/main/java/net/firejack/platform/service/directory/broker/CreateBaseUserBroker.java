...
public abstract class CreateBaseUserBroker<M extends BaseUserModel, DTO extends BaseUser>
        extends OPFSaveBroker<M, DTO, DTO> {
	...
	protected List<ValidationMessage> specificArgumentsValidation(ServiceRequest<DTO> request) throws RuleValidationException {
		...
		DTO userVO = request.getData();
		BaseUserModel user = getStore().findUserByUsername(userVO.getUsername());
		if (user != null) {
			ValidationMessage validationMessage = new ValidationMessage("username", "user.username.not.unique");
			validationMessages.add(validationMessage);
		}
		user = getStore().findUserByEmail(userVO.getEmail());
		if (user != null) {
			ValidationMessage validationMessage = new ValidationMessage("email", "user.email.not.unique");
			validationMessages.add(validationMessage);

		}

		Long registryNodeId = userVO.getRegistryNodeId();
